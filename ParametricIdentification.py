...
from ParametricIdentificationCV import ParametricIdentificationCV, ModelExperimentResult, ModelExperimentParameters, \
...
class ParametricIdentification(ParametricIdentificationCV):
    ...
    def parametric_identification(self, model_tags: list[experiment_model_tags]) -> list[ModelExperimentResult]:
    ...
    def lm(self):
        ...
        self.candidates.append(ModelExperimentResult(ModelExperimentParameters("PI", "lm", lm, True, None), f1))
    ...
    def dtree(self):
        for c in ["entropy", "gini", "log_loss"]:
            for s in ["best", "random"]:
                ...
                self.candidates.append(ModelExperimentResult(ModelExperimentParameters("PI", "dtree", dtree, False, {"criterion": c, "splitter": s}), f1))
    ...
    def gbc(self, par="n_estimators"):
        ...
        if par == "n_estimators":
            for n in [50, 100, 200, 300, 400, 500]:
                ...
                self.candidates.append(ModelExperimentResult(ModelExperimentParameters("PI", "gbc", gbc, False, {"n_estimators": n}), f1))

        elif par == "subsample":
            for s in [0.1, 0.5, 0.75, 0.9, 1.0]:
                gbc = GradientBoostingClassifier(loss="log_loss", subsample=s)
                f1 = self.parametric_identification_cv(gbc, False)
                self.candidates.append(ModelExperimentResult(ModelExperimentParameters("PI", "gbc", gbc, False, {"subsample": s}), f1))

        elif par == "min_sample_split":
            for mss in [2, 3, 5, 10]:
                gbc = GradientBoostingClassifier(loss="log_loss", min_samples_split=mss)
                f1 = self.parametric_identification_cv(gbc, False)
                self.candidates.append(ModelExperimentResult(ModelExperimentParameters("PI", "gbc", gbc, False, {"min_sample_split": mss}), f1))

        elif par == "max_depth":
            for md in [2, 3, 4, 5, 10]:
                gbc = GradientBoostingClassifier(loss="log_loss", max_depth=md)
                f1 = self.parametric_identification_cv(gbc, False)
                self.candidates.append(ModelExperimentResult(ModelExperimentParameters("PI", "gbc", gbc, False, {"max_depth": md}), f1))
